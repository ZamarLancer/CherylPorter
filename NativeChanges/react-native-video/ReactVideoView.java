...
public class ReactVideoView extends ScalableVideoView implements
    ...
    public void setPitchModifier(final float pitch){
        mPitch=pitch;
        if (mMediaPlayerValid) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                    try {
                        ...
                        Log.e(ReactVideoViewManager.REACT_CLASS, "Unable to set pitch, unsupported on this device");
                    }
            } else {
                Log.e(ReactVideoViewManager.REACT_CLASS, "Setting playback pitch is not yet supported on Android versions below 6.0");
            }
        }
    }
